...
Snapshots: `snapshots/briefs.json` (5-min freshness; fallback is `scripts/query-plans.sh --kind=task,brief`). `snapshots/debt.json` for the debt-gate filter (fallback: `scripts/query-plans.sh --kind=debt`).
