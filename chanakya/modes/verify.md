...
1. **Generate test-flow.** Run Test-Flow mode (unless `--round N` points to an existing round). Post-migration: test-flow emits `plans/rounds/<round-id>.yaml` (schema: `_shared/schemas/round.md`, `round@1.0.0`) plus the legacy markdown walkthrough at `plans/user-testing-rounds/user-testing-round<N>.md` during the Phase 2.6 transition.
2. **Prompt the user:**
...
4. **On 'done':** Read the round. Post-migration: read `plans/rounds/<round-id>.yaml` — the `cases[].result` enum (`pending` | `pass` | `fail`) and `summary` object give a typed view with zero parsing. Legacy fallback reads the markdown round file (`[x] pass` checkboxes + `Notes:` blocks) with a `legacy_artifact_read` event emitted so the transition is visible.

   Check completion:
   - If every case has `result: pass` (post-migration) or `[x] pass` (legacy) → invoke Test-Flow `--promote` to generate pre-checked `user-testing.md`, then Review-Feedback mode to transition task state to `verified` (per `_shared/state-machines/task-lifecycle.md`). Round state transitions `open → closed` when review-feedback's closure step fires. Report: "Verified N tasks. Feature wrap-up check running..."
