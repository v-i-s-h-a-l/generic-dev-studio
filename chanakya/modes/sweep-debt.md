...
Snapshots: `snapshots/debt.json` for counter state (5-min freshness; fallback: `scripts/query-plans.sh --kind=debt`). `snapshots/briefs.json` for the pending-test-task selection (5-min freshness; fallback: `scripts/query-plans.sh --kind=task`).

## Steps

1. **Read all three debt counters** from `plans/build-debt.yaml` (or the snapshot above).
