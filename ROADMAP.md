...
- **Skill distribution arc — NEXT UP (2026-04-25, two phases, #167–#172)** — Studio becomes a skill package manager. **Phase 1 (#167+#168+#172):** Skill Authoring Standard + multi-host symlink-farm distribution + migrate studio agents (Achilles/Argus/Chanakya/studio router) to portable standard. End state: typing `/achilles T347` in Codex TUI dispatches identically to Claude Code; "any new adapted host gets all studio skills with one sync" verified via synthetic-host test. **Phase 2 (#169→#170→#171):** Recipe system MVP (curated iOS profile + install/update + PR-based auto-update + migrate existing `~/.claude/skills/` Swift/SwiftUI personal skills + `imported` sub-strategy for unknown-upstream locals) → composites/wizard/LaunchAgent/trust-mode → Tier 2 synthesis composites (parking). End state: every existing skill auto-managed across all hosts; adding a new model = one capabilities.yaml; adding a new skill = one recipe. Phase 1 is the immediate next-up execution; parallelism within Phase 1 deprioritized per user direction.
