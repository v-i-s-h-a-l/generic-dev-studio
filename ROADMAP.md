...
### Planned
- **Forge reliability + bug-fix freeze** — Current active track. Use [`FORGE-RELIABILITY.md`](FORGE-RELIABILITY.md) as the curated lookup before scraping the full issue list. No new feature arc starts until the known bug queue is cleared or explicitly waived by the user. This includes Phase 2.7 knowledge work, host-agnostic Chanakya expansion, Build-opt v2, Lu Ban, dashboard, and new mode packs. First priority is correctness of the existing Forge/Field safety floor: canonical YAML artifact writes, inbox-sweep completeness, Argus/review gate reliability, dispatch correctness, issue graph hygiene, duplicate/blocked-by/caused-by links, and quantitative reliability reporting. Exit criteria are capability-based, not calendar-based: zero open `urgent` bugs, no open Forge-safety bug that can strand work or hide artifacts, measurable reliability improvements, and no unsupported planning claims.
- **Forge reliability + orchestration dogfood arc** — After the bug-fix freeze clears the existing correctness issues, make "Forge" the repo's official control-plane term and use Forge work to prove orchestration automation before Field rollout. Focus areas: conversation/web-only Forge intake; key-tap issue triage; GitHub issue graph hygiene (epic/child, duplicate, blocked-by/blocks, caused-by); quantitative quality reporting; model-role resolver for `forge-architect` / `forge-verifier` / `forge-editor` / `forge-triage`; and a host-agnostic wrapper plan for studio/capture behavior that is currently Claude-shaped. Exit criteria are capability-based, not calendar-based: measurable accuracy/reliability gains, no unsupported planning claims, and reusable patterns ready to harden for Field.
- **Phase 2.7** — Knowledge layer. Deferred until Forge reliability and the known-bug queue are stable. Scope remains `_shared/project-memory.md` + `scripts/memory-query.sh`, Chanakya `modes/knowledge.md` for synthesis, Slack-ingest index joins, and cross-refs from Lu Ban / Achilles / Argus. Every new mode-pack authored here ships with a test-mode-pack fixture (per Phase 2.6.6).
- **Host-agnostic Chanakya v2 (#141)** — Follow-up to host-agnostic-workers-v1. The target is a smaller, session-bound Chanakya coordinator with extracted helpers for repeatable jobs, not a monolithic portability rewrite. See `ARCHITECTURE.md` §Host-agnostic Chanakya v2 plan for the mode inventory, extraction candidates, and conformance gate. Deferred until Forge reliability and the known-bug queue are stable.
- **Build-opt v2 (B5–B6) — agent-friendly Xcode tooling (2026-04-25, #173 → #174)** — Deferred during the bug-fix freeze unless needed to fix an existing reliability bug. Follow-on to B1–B4. **B5 (#173):** swap direct `xcodebuild` calls in Achilles + Argus to [`xcodebuildmcp`](https://github.com/getsentry/XcodeBuildMCP) CLI mode (not MCP server). Same flags, same exit-code contract, structured JSON output enriches `build_check_failed` events with error-level detail. ~3 call sites + 1 line in `scripts/bootstrap-node.sh`. AXe ships bundled. **B6 (#174):** Argus adopts the bundled AXe for a11y-tree UI verification post-test — tap-by-label assertions, `describe-ui` diffs, deterministic gesture chains. Net-new verification layer (today's xcresult parse only sees what XCUITest assertions caught). The multi-machine routing substrate from B1–B4 is the outer layer and is unchanged — `xcodebuildmcp` replaces the inner build call only. #173 pilotable as a single focused session; #174 depends on #173.
...
- Forge reliability + known-bug cleanup gates all new feature arcs. Do not start Phase 2.7, Host-agnostic Chanakya v2, Build-opt v2, Lu Ban, Phase 3 automation, dashboard, or new mode packs while urgent bugs or Forge-safety correctness bugs remain open, unless the user explicitly waives the freeze for a named issue.
- 2.7 and 3 are parallelizable only after the bug-fix freeze clears.
