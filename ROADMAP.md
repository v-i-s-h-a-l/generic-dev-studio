...
- **Phase 2.6.7** ✓ Studio self-hosting + planning-quality layer. Vendor-shipped `studio` as a project-scoped skill at `.claude/skills/studio/` (no more global symlink drift). `scripts/install.sh` + `scripts/verify-install.sh` ship for globally-installed agents. Studio modes `audit` (A1–A3) and `guard` (G1–G3) backed by `scripts/studio-audit.sh` + `scripts/studio-guard.sh` — grep-only, sub-second. `hooks/session-start` auto-invokes audit silently; findings surface only on drift. R10 Iron Law extended from debrief-level to plan-level claims.
- **Build-opt track (B1–B4)** ✓ Mac mini as a remote build node landed. `scripts/swift-test-gate.sh` for package-only fast paths; `scripts/node-dispatch.sh` + `node-health.sh` + `node-pick.sh` route swift-test and M/L `xcodebuild` to the mini; snapshot-sync canonical environment makes the mini the reference machine for tests. Achilles Step 6 routes through the node registry. Closes #110–#113.
- **Worker-node onboarding** ✓ One-shot `scripts/bootstrap-node.sh` + `scripts/bootstrap.sh` unified wizard for manager / worker / dual roles. Control-plane scripts (`sync-worker.sh`, `sync-workers-all.sh`, `configure.sh`) + manifest. Docs split laptop vs node prereqs explicitly. Closes #125.
- **Skill distribution arc** ✓ Studio becomes a skill package manager. Phase 1 (#167 + #168 + #172): Skill Authoring Standard + multi-host symlink-farm distribution + studio agents migrated to portable standard. Phase 2 (#169): recipe system MVP (curated iOS profile, install/update scripts, PR-based auto-update). Deferred: #170 (composites/wizard/LaunchAgent) + #171 (Tier 2 synthesis composites, parking-lot).
- **Forge reliability + bug-fix freeze** ✓ Archived / waived as the active gate on 2026-05-03. Safety-floor regressions that could strand work, hide artifacts, bypass review, or make status lie were closed; the user explicitly waived the freeze as the current blocker. Historical lookup: [`archive/forge-reliability-2026-05-03.md`](archive/forge-reliability-2026-05-03.md).

### Planned
- **Forge reliability + orchestration dogfood arc** — Current active direction. Make "Forge" the repo's official control-plane term and use Forge work to prove orchestration automation before Field rollout. First pilot: [#476](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/476), the autonomous continuation loop with decision escrow for long-running studio chains. Focus areas: conversation/web-only Forge intake; key-tap issue triage; GitHub issue graph hygiene (epic/child, duplicate, blocked-by/blocks, caused-by); quantitative quality reporting; model-role resolver for `forge-architect` / `forge-verifier` / `forge-editor` / `forge-triage`; and a host-agnostic wrapper plan for studio/capture behavior that is currently Claude-shaped. Exit criteria are capability-based, not calendar-based: measurable accuracy/reliability gains, no unsupported planning claims, and reusable patterns ready to harden for Field.
- **Studio v2 substrate spec strand (#444 A0-A0.6)** — The v2 greenfield substrate is being specified before implementation. The checked-in A0 artifacts include [`core/RESEARCH.md`](core/RESEARCH.md), which maps OSS agent-framework research to the 12 Studio v2 principles; [`core/HOST-CAPABILITY-MATRIX.md`](core/HOST-CAPABILITY-MATRIX.md), which defines the A0a host matrix; [`core/EVENT-LOG-SEMANTICS.md`](core/EVENT-LOG-SEMANTICS.md), which specifies durable event-log ordering, dedupe, replay, lock boundaries, and backpressure; and [`core/AUTH-PERMISSIONS.md`](core/AUTH-PERMISSIONS.md), which defines GitHub, local-tool, delegated-process, release-action, and loud-failure semantics for the later A0.5 `SPEC.md`.
- **Phase 2.7** — Knowledge layer. Ready to plan once the orchestration dogfood slice is shaped. Scope remains `_shared/project-memory.md` + `scripts/memory-query.sh`, Chanakya `modes/knowledge.md` for synthesis, Slack-ingest index joins, and cross-refs from Lu Ban / Achilles / Argus. Every new mode-pack authored here ships with a test-mode-pack fixture (per Phase 2.6.6).
- **Host-agnostic Chanakya v2 (#418)** — Follow-up to host-agnostic-workers-v1. The target is a smaller, session-bound Chanakya coordinator with extracted helpers for repeatable jobs, not a monolithic portability rewrite. See `ARCHITECTURE.md` §Host-agnostic Chanakya v2 plan for the mode inventory, extraction candidates, and conformance gate.
- **Build-opt v2 (B5–B6) — agent-friendly Xcode tooling (2026-04-25, #173 → #174)** — Follow-on to B1–B4. **B5 (#173):** swap direct `xcodebuild` calls in Achilles + Argus to [`xcodebuildmcp`](https://github.com/getsentry/XcodeBuildMCP) CLI mode (not MCP server). Same flags, same exit-code contract, structured JSON output enriches `build_check_failed` events with error-level detail. ~3 call sites + 1 line in `scripts/bootstrap-node.sh`. AXe ships bundled. **B6 (#174):** Argus adopts the bundled AXe for a11y-tree UI verification post-test — tap-by-label assertions, `describe-ui` diffs, deterministic gesture chains. Net-new verification layer (today's xcresult parse only sees what XCUITest assertions caught). The multi-machine routing substrate from B1–B4 is the outer layer and is unchanged — `xcodebuildmcp` replaces the inner build call only. #173 pilotable as a single focused session; #174 depends on #173.
...
- New Forge-safety regressions can still preempt roadmap work when they can strand work, hide artifacts, bypass review, or make status lie, but the 2026-05 Forge reliability freeze is no longer the active gate.
- 2.7 and 3 are parallelizable after the orchestration dogfood slice proves the long-running control-plane loop.
