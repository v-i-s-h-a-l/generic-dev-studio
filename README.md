...
/dev-studio manager composite-chain run --manifest composite.yaml # selected MVP equivalent to --composite-manifest; plans/runs sequential children until complete or blocked
...
/dev-studio manager composite-chain resume --run-id <run_id> --continue # clean-session continuation until the chain completes or needs confirmation
...
scripts/manager-composite-chain.sh run --manifest composite.yaml --json # script equivalent for explicit composite-chain execution
...
scripts/lint-html-theme.sh --staged                         # keep generated HTML pages system-theme aware by default
...
scripts/studio-ios-check-router.sh explain --operation release:testflight --role release --requires-secret-scope asc,slack # release/TestFlight capability + secret + priority routing explanation
scripts/studio-ios-check-failover.sh decide --operation build --task-id T001 --selected-executor worker-a --failure-signal remote_timeout # classify worker-routed check failure and publish a finite retry/halt path
...
scripts/pr-headless-review.sh <pr>                          # run smoke-eligible no-secret reviewer gate; target repo PRs stop before merge by default
...
core/v2/skills/dev-studio/
  ...
  docs.html        # self-contained /dev-studio docs page with per-role help and system theme support
...
scripts/                # multi-worker fleet (BETA)
  ...
  manager-composite-chain.sh # explicit sequential composite-chain manifest run/status/resume front door; step commands remain available for recovery and status is non-mutating
  ...
  issue-context-packet.sh # public-safe issue body + comment packet for comment-aware planning; comments inform decisions but issue bodies/manifests/state/reviewed plans remain authoritative
  studio-comment.sh    # structured `studio-comment:v1` issue/PR comment writer with idempotent update and public-safety checks
  lint-studio-comments.sh # pre-commit comment-structure gate for new unstructured issue/pr comments; bypass is explicit/user-controlled only
  ...
  studio-ios-check-router.sh # local-manager-first iOS scheduler with build/test affinity plus release/TestFlight secret-scope priority routing
  studio-ios-check-failover.sh # typed worker-routed build/test failure classification, finite retry/halt policy, and partial-artifact retention
...
`--include-comments` with either `--issue <n>` or `--issue-set <csv>`. That
switch routes planning through `scripts/issue-context-packet.sh`, records the
packet and `packet.json` sidecar in the plan-chain artifacts, and lets comments
inform decisions, constraints, acceptance changes, conflicts, and open
questions. Comments are still planning signals only; issue bodies, manifests,
runtime state, event logs, reviewed plan artifacts, and worker summaries remain
the source of truth.
...
The selected MVP equivalent to a `work-chain --composite-manifest <file>` flag
is `/dev-studio manager composite-chain run --manifest <file>` or
`scripts/manager-composite-chain.sh run --manifest <file>`. Inputs must be an
...
child planning and resume. `run --manifest` initializes state, plans the active
child, executes it, and continues through later children until the composite
chain completes or a child needs confirmation/recovery. Use `/dev-studio
manager composite-chain status --run-id <run_id>` for non-mutating progress
inspection, then continue from a clean session with `/dev-studio manager
composite-chain resume --run-id <run_id> --continue`. For focused recovery, use
`manager composite-chain init --manifest <file>`, `plan-active-child`,
`execute-active-child`, or plain `resume` as the status output directs.
Sequential mode plans only the next
