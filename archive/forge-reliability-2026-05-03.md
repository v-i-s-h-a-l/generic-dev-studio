...
Archived snapshot. This file preserves the pre-archive planning lookup and may contain issue statuses that are stale relative to live GitHub. Do not update routine state drift here; use GitHub Issues and the current planning surface instead.

This was the curated lookup for the Forge reliability path before archival.
...
| 6 | [#418](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/418) Host-agnostic Chanakya v2 implementation | Makes orchestrator dispatch and status paths portable without Claude-Code-only load-bearing primitives. |
| 7 | [#203](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/203) Achilles modes/debug.md + bug-fix flow rewrite | Sets up the bug-fix workflow and localization loop that will make future repair work faster and less brittle. |
| 8 | [#213](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/213) Achilles SourceKit-LSP symbol localization | Improves bug localization from file-level grep to symbol-level Swift resolution once the safety-floor queue has room for capability work. |
| 9 | [#257](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/257) Argus change-classifier for selective rule loading | Reduces review latency and token use by loading only relevant review rules for a diff. |
| 10 | [#256](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/256) Achilles brief-summary primitive | Adds compact brief slices for cheap reads and dispatch/status decisions. |
...
| [#434](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/434) Build gate success marker can be absent without diagnostics | Closed | Reliability bug | Full-green tasks can be blocked by opaque harness failure with no actionable build diagnostic. |
| [#435](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/435) TestFlight push can preserve a rejected marketing version | Closed | Reliability bug | Release tooling can contradict explicit version intent and upload a rejected version without a preflight halt. |
