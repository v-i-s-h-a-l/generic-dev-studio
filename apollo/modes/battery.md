...
description: Diagnose → measure → propose → patch → re-measure protocol for battery / energy regressions under strict-9 evidence gating.
type: mode-pack
schema_version: 1
snapshots: []
budget_tokens: 6000
...
| Networking radio drain | Chatty / un-coalesced traffic prevents radio sleep; cellular radio cost dominates because tail-energy fires on every wake; persistent connections poke the radio without batching | Battery owns the energy claim: Power Profiler `network` row, ASC power attribution, or MetricKit energy row. Request / connection evidence comes from `apollo/_shared/primitives/evidence-gate.md §Network hard-evidence catalogue`: paired Network / HTTP Traffic trace, `MXNetworkTransferMetric` + `MXCellularConditionMetric` for cellular claims, or ASC network-attributed rows | Power Profiler + Network instrument |
...
| Networking radio drain | ≥ 5 minutes Power Profiler paired with Network instrument running concurrently; ≥ 10 connection wakes captured in the timeline; request/connection citation fields come from the shared network catalogue | ≥ 7 days `MXNetworkTransferMetric` payload window from the same carrier-class cohort, paired with `MXCellularConditionMetric` for cellular claims |
...
| Is the radio waking too often or staying awake too long? | Power Profiler + Network instrument | `OSSignposter` interval `<Network-batch>` brackets the request burst; event payload at each `URLSessionTask` `didCompleteWithError` / WS frame boundary | Power Profiler `network` row + Network / HTTP Traffic `.trace` showing RTT-aware connection wake distribution; `MXNetworkTransferMetric.cumulativeCellularUpload` / `cumulativeCellularDownload` carrier-class cohort delta paired with cellular-condition context |
...
Hard-evidence catalogue rows for battery (from `apollo/_shared/primitives/evidence-gate.md §Hard-evidence catalogue`). Networking radio drain additionally cites `apollo/_shared/primitives/evidence-gate.md §Network hard-evidence catalogue`; the rows below are pointers to that shared network standard, not a parallel acceptance rule.
...
| Network / HTTP Traffic `.trace` | URLSession tasks / transactions, per-connection bytes, RTT, retransmits, TCP / cellular activation cadence | networking radio drain (paired with Power Profiler; citation fields come from the shared network catalogue) |
...
| `MXNetworkTransferMetric` + `MXCellularConditionMetric` when cellular | `cumulativeWifiUpload` / `cumulativeWifiDownload` / `cumulativeCellularUpload` / `cumulativeCellularDownload` per cohort; `histogrammedCellularConditionTime` for carrier-class context | networking radio drain (shared network catalogue) |
...
| Soft / no evidence after auto-capture-before-refuse exhausts the tree | Emit `apollo_refused` with verbatim refusal block from `evidence-gate.md §Refusal protocol`. The unblock recipes section must list **at minimum**: `Power Profiler trace` (RUN `xctrace record --template "Power Profiler" …`) AND `MXAppRunTimeMetric.cumulativeForegroundEnergy` payload (RUN MetricKit subscription / ASC export). For networking radio drain, also include the unblock recipes from `evidence-gate.md §Network hard-evidence catalogue` (`Network` trace, signposted `URLSessionTaskMetrics`, MetricKit network + cellular-condition payloads, or ASC row). One accepted catalogue artifact is required for any battery fix recommendation |
...
| Carrier-class mismatch on networking-class captures (`MXCellularConditionMetric.histogrammedCellularConditionTime` distribution differs) | `MXCellularConditionMetric` | Cellular drain depends on radio condition; pre-fix on full-bars vs post-fix on edge-coverage hides the regression |
...
| Soft network evidence — screenshot, one-off complaint, logs without cohort / scenario, trace without signposts, or MetricKit transfer payload without cellular-condition context | permanent | Refuse per `evidence-gate.md §Network soft-evidence and refusal cases`; unblock with Network / HTTP Traffic `.trace`, signposted `URLSessionTaskMetrics`, `MXNetworkTransferMetric` + `MXCellularConditionMetric`, or ASC row |
