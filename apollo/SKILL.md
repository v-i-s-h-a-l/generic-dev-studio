...
|---|---|---|
| `memory` / "investigate memory regression" / "leak" / "OOM" | `modes/memory.md` | Stage 2a — #230, not yet shipped |
| `thermal` / "thermal throttling" / "device heat" | `modes/thermal.md` | Stage 2b — #231, not yet shipped |
...
Mode packs emit scoped events through `scripts/emit-event.sh`. Names ship with the first mode pack (#230); the catalogue lives at `_shared/contracts/events.md`. Anticipated names: `apollo_capture_started`, `apollo_capture_completed`, `apollo_recommendation`, `apollo_refused`, `apollo_advisory`, `apollo_capture_deferred`. Every event carries `mode`, `artifact_shape`, `cohort`, and (when applicable) `regression_id`.
