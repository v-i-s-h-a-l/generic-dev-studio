...
#
# <schema-name>: worker-report | debrief | review-verdict | handoff | chain-task-envelope | apollo-scenario | apollo-code-area | apollo-signpost-plan
...
case "$SCHEMA_NAME" in
  ...
  *)
    printf 'validate-contract: unknown schema %s (want worker-report|debrief|review-verdict|handoff|chain-task-envelope|apollo-scenario|apollo-code-area|apollo-signpost-plan)\n' \
      "$SCHEMA_NAME" >&2
    exit 2
    ;;
esac
