...
scripts/studio-comment.sh --dry-run --target issue:758 --kind chain-progress --idempotency-key chain:issue-758:chain-progress --summary "Progress summary" # render a structured studio-comment:v1 body
scripts/lint-studio-comments.sh --staged                          # pre-commit comment-structure gate; blocks new unstructured issue/pr comments outside the shared writer
scripts/manager-composite-chain.sh run --manifest composite.yaml --json     # selected MVP equivalent to --composite-manifest; plan/run sequential children until complete or blocked
scripts/manager-composite-chain.sh status --run-id <run_id>                 # non-mutating composite-chain status and next resume command
scripts/manager-composite-chain.sh resume --run-id <run_id> --continue      # clean-session continuation until the chain completes or needs confirmation
...
# Non-release chains treat expected_source_sha/base_sha as provenance: if the base advances, finalization rebases onto the live base and records planned/finalized base SHAs; release-bearing chains keep the strict source-SHA gate.
# Comment-aware planning is for issues whose public comments contain decisions, constraints, failures, acceptance changes, conflicts, or open questions that should inform the next reviewed plan. Use `--include-comments` only with `--issue` or `--issue-set`; the packet builder archives raw reads privately and feeds planners the bounded packet/sidecar instead. Comments never override issue bodies, manifests, state files, event logs, reviewed phase artifacts, or worker summaries.
# Public studio issue/PR comments must go through `scripts/studio-comment.sh`, which emits the first-line `studio-comment:v1` marker and updates an existing matching idempotency key. `scripts/lint-studio-comments.sh --staged` is the pre-commit comment-structure gate; new unstructured `issue comment` / `pr comment` calls are blocked unless covered by the per-line migration annotation or the explicit user-controlled `STUDIO_BYPASS_COMMENT_STRUCTURE_LINT=1` escape hatch.
# Composite parent-chain mode is a manager/router addition beside plan-chain and work-chain. The selected MVP equivalent to a `work-chain --composite-manifest <file>` flag is `scripts/manager-composite-chain.sh run --manifest <file>` or `/dev-studio manager composite-chain run --manifest <file>`; parent-issue natural-language extraction and `work-chain --composite-parent <issue>` are future/non-MVP. Use `status --run-id <run_id>` for non-mutating progress inspection, then continue from a clean session with `resume --run-id <run_id> --continue`; `manager composite-chain init --manifest <file>`, `plan-active-child`, `execute-active-child`, and plain `resume` remain recovery/debug controls as the status output directs. Sequential mode plans only the next eligible child after prior children complete or skip; existing gates remain in force for each child: plan review, issue creation, worker execution, verification, PR review/merge policy, and closeout.
# Chain manifest preflight rejects planning artifacts before run creation; external manifests resolve target repo from explicit config, then the caller checkout, before using the manifest directory.
