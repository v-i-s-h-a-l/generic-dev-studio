...
- Non-interactive GitHub push works for the active branch: `studio_git_transport_push --dry-run --porcelain -u origin HEAD` (sourced from `scripts/lib-github-transport.sh`) succeeds. The helper proves `gh auth status` under the normalized `github_home`, overrides any stale ambient `credential.helper`, and emits `gh_missing`, `gh_auth_missing`, `credential_helper_stale`, or `network_partition` on failure. This preflight runs before any build-number or version mutation; auth failures must not open a credential prompt or create a stranded release commit. The halt-failed remediation hint is `scripts/studio-gh.sh auth status && ( cd "$PROJECT_ROOT" && studio_git_transport_push --dry-run -u origin HEAD )`.
