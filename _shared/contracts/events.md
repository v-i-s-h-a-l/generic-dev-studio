...
| `review_pending` | Emitted by Chanakya in Step 0A when a processed debrief's `## Argus Review` section indicates Argus was skipped / not invoked on a non-exempt task. Cleared implicitly when a later `review_approved` / `review_flagged` / `review_blocked` lands for the same task. When a structured waive exists at `~/.dev-studio/<project>/state/waives/<gate>.yaml`, `sweep-ingest.sh` also bumps that file's `accumulated_count` atomically (see issue #83). | `merge_sha`, `reason` (`argus_skipped_in_debrief`); optional `sunset_on` (string — free-text predicate copied from the active waive file, if any; backward-compatible) |
