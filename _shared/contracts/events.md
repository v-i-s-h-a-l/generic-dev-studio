...
| `merge_deferred_on_flagged` | Achilles attempted a merge under `--require-approved`, but the latest Argus verdict was flagged rather than approved. No merge occurs; Chanakya surfaces the task for a user decision to fix first or explicitly merge flagged work. | `branch`, `reason` (`require_approved`), `override` (`--steal-flagged`) |
