...
| `debrief_missing` | Step 0E0 of the inbox sweep — `scripts/sweep-detect-missing-debriefs.sh` detected a `task_merged` event whose paired debrief never landed in `plans/debriefs/`. Catches the dark window between Step 9 (merge) and Step 10 (debrief emit) when a session crashes, and any `task_merged` produced outside the Achilles pipeline. Stale window: only fires when `task_merged` is older than 600s, so a debrief still being written by Step 10 is not falsely flagged. Idempotent per `(task, merge_sha)` — once a `debrief_missing` has been emitted for that pair, the detector skips re-emit. Surfaced in `/chanakya status` via push-queue. See #249 (Phase 1 — observability; Phase 2 flips Step 9/10 for true pre-merge enforcement). | `merge_sha`, `source_branch` (`achilles/<task-id>` from the `task_merged` event), `age_s` (seconds between `task_merged` ts and detection ts) |
