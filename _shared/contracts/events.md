...
| `task_cancelled` | User cancelled a pending or in-flight task (`scripts/achilles-cancel.sh` compatibility entry point or a worker abort). Paired with no `task_completed`. | `stage` (`pending`\|`in_flight`\|`merged`), `reason` (`user_abort`\|`replaced`\|`stale_brief`\|`other`), `worker` (`worker-N` if known) |
| `task_rescued` | Worker moved a task to `rescue/` — either `timeout` (rc=124 from `gtimeout`) or `silent_stuck` (rc=0 with no debrief written). Emitted by `scripts/achilles-worker.sh` compatibility entry point. Visible equivalent of the rescue file, so the manager can surface + push without polling worker directories. | `reason` (`timeout`\|`silent_stuck`), `timeout_s` (only on timeout), `rc`, `debrief_written` (0\|1), `worker` (`worker-N`) |
...
| `precommit_review_failed` | `scripts/pre-commit-review.sh` could not obtain a usable reviewer verdict because the reviewer path, auth profile, payload handoff, command execution, or verdict parsing failed. This is infrastructure failure, not review approval. | `status` (`failed`), `failure_kind` (`infrastructure`), `reason`, `review_host`, `branch`, `head`, `patch_id`, `payload_storage`, `duration_s` |
...
| `ios_check_failover_decision` | `scripts/studio-ios-check-failover.sh` classified a worker-routed build/test failure and selected a finite retry or halt path. Emitted for wet decisions; compact form only so the event stays under the JSONL atomicity cap. | `operation`, `role`, `chain`, `task_id`, `attempted_executor`, `failure_class` (`worker_unavailable`\|`worker_timeout`\|`remote_command_failed`\|`artifact_missing`\|`artifact_malformed`\|`sync_drift`\|other typed simulator/cache/lock class), `failure_signal`, `selected_retry_path` (`retry_another_worker`\|`retry_local`\|`retry_same_worker_after_source_sync`\|`reclaim_stale_lock_then_retry`\|`quarantine_cache_then_retry_*`\|`halt_*`\|`no_retry_terminal_failure`), `target_executor`, `retry_count`, `max_retries`, `final_outcome` (`retry_planned`\|`terminal_failure`\|`halted`), `retention_class`, `preserve_partial_artifacts`, `idempotency_key` |
| `release_priority_routing_decision` | `scripts/studio-ios-check-router.sh` selected or refused a release/TestFlight executor under capability, secret-scope, and priority routing. Emitted for wet release routing decisions, not dry-run explanations. Task field is the release tag or release task id. | `selected_executor`, `reason_class`, `operation`, `role`, `chain`, `task_id`, `job_class` (`release`), `priority` (`release`), `release` (`channel`, `required_secret_scopes`, `routing_policy`, `operator_approval_required`, `external_mutation_authority`), `queue` (`selected_priority`, `selected_wait_s`, `selected_preemption`), `preemption` (`status`, `refused`, `safe_boundary`, `displaced_count`, `displaced_jobs`, `displaced_work_retains_cache_artifacts`), `economics`, `scheduler` |
...
| `chain_supervisor_decision`, `chain_run_started`, `chain_run_completed`, `chain_started`, `chain_completed`, `chain_issue_started`, `chain_issue_completed`, `chain_issue_merged`, `chain_issue_scheduler_blocked`, `chain_issue_closed`, `chain_pr_opened`, `chain_review_completed`, `chain_resume_attempt_started`, `chain_resume_attempt_completed`, `chain_halt_recorded`, `chain_state_projection_repaired`, `chain_stale_lock_removed`, `chain_decision_escrow_opened`, `chain_artifact_validation_failed`, `chain_worker_summary_ingested`, `chain_telemetry_gap`, `chain_auth_normalized`, `chain_ios_artifact_cleanup_completed`, `checkpoint_auto_created`, `checkpoint_auto_loaded`, `checkpoint_context_savings_estimated` | `scripts/studio-chain-runner.sh` executes or resumes an autonomous issue chain. Shared events keep the legacy `data` shape for readers; the private per-run `events.jsonl` additionally uses the structured envelope in `_shared/contracts/chain-run-telemetry.md`. | `run_id`, `chain_run_id`, `issue_run_id`, `status`, `duration_s`, plus event-specific compact fields such as `action`, `stage`, `attempt_id`, `host`, `verdict`, `reason_id`, `gap_kind`, checkpoint drift/size counters, abstract cleanup counts/classes, and private artifact pointers. |
...
| `test_run_failed` | Tests red | `failing_tests` (array, argus) \| `node`+`scheme`+`test_target`+`duration_s`+`attempt`+`exit_code` (achilles); achilles also uses `reason: locked_out` + `waited_s` for the 30-min lock timeout and the remote-dispatch failure classes from `_shared/contracts/remote-build-dispatch.md`; optional `failover` mirrors the compact build failover object when the failed test was worker-routed |
...
| `argus_rules_skipped` | The reviewer classified the diff and skipped rule packs whose `applies_when` metadata did not match. Event name remains `argus_*` for telemetry compatibility. | `skipped` (array of rule-pack names), `classifier` (object from `scripts/argus-classify-diff.sh`) |
| `argus_gate_skipped` | Reviewer dispatch exited non-zero without a verdict, OR a `task_merged` fired with no preceding `review_(approved\|flagged\|blocked)` for the same task. Loud-skip sentinel for #154. Event name remains `argus_*` for telemetry compatibility. | `stage`, `idem_key`, `reason` (`unknown_host`\|`missing_manifest`\|`missing_spawn_command`\|`secret_scope_floor_unmet`\|`mktemp_failed`\|`validator_unavailable`\|`handoff_schema_violation`\|`verdict_timeout_<N>s`\|`no_verdict_at_merge`\|`unknown_exit_<rc>`), `host` (node id where dispatch was attempted, e.g. `codex-mac-mini-1`; required for infra-broken reasons, optional otherwise), `exit_code` (where applicable) |
| `argus_preflight_failed` | Reviewer dispatch refused before spawn because the selected host could not satisfy the dispatch preflight. Emitted before the terminal `argus_gate_skipped` event so dashboards can count root causes without parsing stderr. Event name remains `argus_*` for telemetry compatibility. | `stage`, `idem_key`, `reason` (`unknown_host`\|`missing_manifest`\|`missing_spawn_command`\|`secret_scope_floor_unmet`), `host`, `detail` |
...
| `task_train_run_started`, `task_train_resume_started`, `task_train_task_started`, `task_train_plan_review_completed`, `task_train_task_dispatched`, `task_train_task_completed`, `task_train_outcome_review_completed`, `task_train_halted`, `task_train_run_completed` | `scripts/chanakya-task-train.sh` runs one manually launched task train with sibling plan/outcome reviews around worker dispatch. Parallel trains are separate user-started sessions, not auto-spawned by the runner. | `run_id`, `train`, `state_dir`, `uuid`, `stage`, `verdict`, `review_file`, `target`, `terminal_event`, `reason` |
| `feedback_ingested` | Feedback record minted from a Slack thread / DM / channel, or studio-feedback inbox routing created/commented/deferred a public issue destination. | Ingest mint: `source`, `channel`, `thread_ts`, `reporter`, `build`. Studio-feedback routing: `source_file`, `destination_issue`, `issue_url`, `disposition` (`created_issue`\|`added_to_existing_issue`\|`deferred_manual_triage`\|`private_only`\|`upstream`; legacy manager-analyze also emits `matched_existing_issue`\|`consolidated_with_related_feedback`), optional `triage_reason` or `policy_reason`. |
...
| `appstore_submission_failed` | `scripts/studio-tf-push.sh appstore` attempted `POST /v1/appStoreVersionSubmissions` and got a non-2xx response (or 409 with no recoverable existing-submission id). Halts before any user-visible "ready" output, Slack post, PR creation, artifact write, pending-marker write, **or git-tag + GitHub-draft-release creation** — the tag/draft steps are deferred until after the submission POST succeeds (#824 follow-up), so a failed submission leaves no orphaned tag or draft on GitHub. (#824) | `version_id`, `build_id`, `http_status`, `reason`, `tag` (the would-be tag — useful for correlation; no real tag is created on the failure path) |
...
| `release_attempt_state_changed` | Nabu changed a resumable operation-level release attempt state. Task field = release attempt id. | `from`, `to`, `actor`, `event_id` |
| `release_attempt_transaction_appended` | Nabu or an adapter appended an intent, side-effect, skip, failure, transition, or resume entry to a release attempt transaction log. Task field = release attempt id. | `entry_type`, `status`, `actor`, `event_id` |
...
Emitted by `scripts/chanakya-snap.sh` compatibility producer and by role payloads that consume snapshots. See `_shared/patterns/router-pattern.md` §Freshness and fallback for the contract. Agent field remains `chanakya` for legacy event compatibility.
...
| `chain_issue_merged` | One issue leaf has been integrated into the chain branch. | `run_id`, `chain_run_id`, `issue_run_id`, `chain`, `branch`, `issue_branch`, `commit_after`, `status` |
| `chain_issue_closed` | The runner closes or comments on an integrated source issue after the chain PR path succeeds. | `run_id`, `chain_run_id`, `issue_run_id`, `issue_number`, `pr_url`, `status` |
...
| `chain_completed` | One manifest chain finishes its PR path. | `run_id`, `chain_run_id`, `chain`, `pr_url`, `commit_after`, `duration_s` |
