...
| `build_check_failed` | Step 6 build gate red — blocks merge; task left in-flight for the user to resolve | `mode`, `attempt`, `errors`, `warnings` (full-green), `reason` (`locked_out` iff lock wait exceeded; `xcode_major_drift` per #136; `success_marker_absent` per #265 — xcodebuild exited 0 without `** BUILD SUCCEEDED **` in the log, signalling silent success on unresolvable destination or other catalog-resolution failure), `scheme` (full-green) |
...
| `build_queue_position` | Full-green build gate enqueued a waiter on the per-node FIFO queue (#266 / #218 Stage A). Emitted exactly once per gate invocation at enqueue time, before any wait. `position == 1` means we entered as head (no wait expected); higher values mean N-1 entries are ahead. Joins to the same task's `build_check_*` events via `task` + `studio.dispatch.*` tags. | `mode` (`full-green`), `node` (same value as `studio.dispatch.node`), `position` (1-based; 1 = head), `depth` (total entries in queue including ours at the moment of enqueue), `attempt` |
...
| `legacy_artifact_read` | A runtime script fell back to a pre-Phase-2.6 path because the post-2.6 ledger shape was unavailable (`plans/index.yaml` missing, `plans/debriefs/` absent, `yq` unavailable on the machine, or no YAML brief carries the requested `legacy_task_id`). One emit per sweep per domain, or one emit per task-load for the per-task loader. Task field is empty for sweep-scoped domains; populated for `task-load-spec.sh`. | `domain` (`briefs`\|`debriefs`\|`candidates`), `reason` (`plans_index_missing`\|`plans_debriefs_missing`\|`yq_unavailable`\|`no_yaml_brief_for_legacy_id`\|`legacy_debrief_ingest`), `caller` (script name) |
...
| `debrief_missing` | Two emitters share this event under #249/#300. **Phase 1 (post-hoc detection)** — Step 0E0 of the inbox sweep, `scripts/sweep-detect-missing-debriefs.sh`, detects a `task_merged` event whose paired debrief never landed in `plans/debriefs/`. Catches `task_merged` produced outside the Achilles pipeline or by a manual-mode session that bypassed Step 9–10.5. Stale window: only fires when `task_merged` is older than 600s. Idempotent per `(task, merge_sha)`. Reason field is implicit (no `reason` key) for back-compat. **Pre-merge compatibility signal** — `scripts/task-merge.sh` emits this event with `reason: pre_merge_warn` only when the composite merge safety gate allows a merge with build + review present but the debrief signal absent. Blocks now emit `merge_safety_blocked` instead. Both paths surface in `/chanakya status` via push-queue. | Phase 1: `merge_sha`, `source_branch` (`achilles/<task-id>`), `age_s`. Pre-merge warning: `reason` (`pre_merge_warn`), `branch` (`achilles/<task-id>`). |
| `merge_safety_blocked` | `scripts/task-merge.sh` composite safety gate found two or more missing pre-merge signals and refused to merge. Prevents the multi-failure cascade where build, review, and debrief safeguards all degrade independently. | `task_id`, `branch`, `missing_signals` (`build_passed`, `review_verdict`, `debrief`), `signal_count` (present signals), `missing_count` |
| `merge_safety_warn` | `scripts/task-merge.sh` found exactly one missing pre-merge safety signal and allowed the merge to continue. Debrief-only warnings also emit the legacy `debrief_missing{reason:pre_merge_warn}` event for existing inbox-sweep surfacing. | `task_id`, `branch`, `missing_signals`, `signal_count`, `missing_count` |
| `merge_safety_override` | Caller passed `--force` to `scripts/task-merge.sh` after the composite safety gate found two or more missing signals. The merge proceeds, but the override is explicit in the event log. | `task_id`, `branch`, `missing_signals`, `signal_count`, `missing_count` |
