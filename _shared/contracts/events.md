...
| `feedback_ingested` | Feedback record minted from a Slack thread / DM / channel, or manager analyze routed a studio-feedback inbox record to a durable public issue destination. | Ingest mint: `source`, `channel`, `thread_ts`, `reporter`, `build`. Manager analyze routing: `source_file`, `destination_issue`, `issue_url`, `disposition` (`matched_existing_issue`\|`consolidated_with_related_feedback`\|`created_issue`), optional `policy_reason`. |
