...
| 2 | **Debrief** | Achilles | Chanakya `inbox-sweep` (state: `emitted → ingested → superseded`) | Chanakya, knowledge layer (Phase 2.7) | [`schemas/debrief.md`](../schemas/debrief.md) — `debrief@2.1.0` | `plans/debriefs/<debrief-id>.yaml` | (state field embedded in schema, §`state` enum) | 1 per task or direct-debrief |
