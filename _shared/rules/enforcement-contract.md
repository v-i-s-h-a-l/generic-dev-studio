...
| `E_SURFACE_REMOVED` | An entry disappears from `docs-surface.json` between HEAD and staged | Removing a sub-command or command is a breaking change per RELEASES.md. Either restore the entry or escalate to ask-tier review before proceeding. User-controlled override after explicit approval: `STUDIO_ALLOW_SURFACE_REMOVALS=1`. |
