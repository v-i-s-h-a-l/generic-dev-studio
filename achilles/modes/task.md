...
**Package-only fast path (#110).** Before the size-driven gate, try `swift-test-gate.sh`. When the diff lives entirely under a single SPM package directory it runs `swift test --package-path <pkg>` (no simulator, no xcodebuild lock) and the verdict stands; otherwise it exits 1 and the size-driven gate runs as the fallback. If `swift-test-gate.sh` exits 2 with `reason: focused_verification_structurally_blocked`, stop and report the structural blocker directly; do not run `task-build-gate.sh` unless the user explicitly authorizes the broader run. Skipped under `--force-build` since the user is opting in to xcodebuild explicitly.
...
if [ "${FORCE_BUILD:-0}" = "0" ]; then
  ...
  case $rc in
    ...
    *) exit $rc ;;  # Includes focused_verification_structurally_blocked: no automatic heavy-runner fallback.
  esac
else
...
The script emits `build_check_started` on entry and `build_check_passed` / `build_check_failed` on exit, or `build_check_aborted` if it exits between start and the normal terminal (arg failure, signal, unhandled exception — see events.md #106). `build_check_started` carries an `attempt` counter: 1 for a cold start, 2+ when a prior build-check for the same task emitted `started` without a paired terminal (e.g. a process that died, then got re-invoked). The counter resets on any terminal event. Full-green owns the atomic `mkdir`-based xcodebuild slot lock under `~/.dev-studio/.runtime/xcodebuild-lock/<node-id>/slot-<n>/` — scoped per dispatch target so a laptop-local build and a mini-dispatched build don't serialize on each other — with 45-minute per-slot staleness reclaim, per-task `-derivedDataPath`, and a trap that releases the slot on any exit. Slot count comes from the node's `parallel_build_slots` field in `nodes.json` (default 1; #218 Stage C / #268) — at 1 it's a single-holder lock pinned to `slot-1/`, at N>1 up to N concurrent holders run on distinct slot subdirs. The lock is fed by a per-node priority queue at `~/.dev-studio/.runtime/build-queue/<node-id>/` (#266 / #218): every full-green invocation enqueues an entry on entry and waits until it is in the first `parallel_build_slots` positions before attempting `mkdir` on a slot. `release` entries from TF/App Store builds sort ahead of queued `task` and `background` entries, but in-flight lock holders keep their slots until done. The queue emits `build_queue_position` once per invocation with `{position, depth, slots, priority}` and `build_queue_promoted` when release work jumps ahead of older lower-priority queued work; orphaned entries get GC'd after 45 min, matching the lock's own staleness threshold. A separate per-task lock under `<project>/.runtime/state/build-check-locks/<task-id>/` (60-min staleness reclaim) refuses concurrent invocations for the same task — exit `4`, no `started`/`aborted` emitted (#209). Exit codes: `0` green, `2` red, `3` locked-out (30-minute wait exceeded — applies to both queue head-wait and inner mkdir-lock acquire), `4` refused (concurrent same-task invocation).

**Node dispatch (#112, #137).** Both `swift-test-gate.sh` and `task-build-gate.sh` full-green mode route through `scripts/node-pick.sh` — the swift-test gate asks for a `swift-test`-tagged node, the full-green gate asks for `xcodebuild`. When a healthy remote node answers (see `~/.dev-studio/.runtime/nodes.json`), the compile + test cost lands on that node over SSH. When no remote is registered, reachable, or role-matching, `node-pick` returns `local` and the gate runs on the laptop with no behavioural change. Every fallback emits a discriminating `node_fallback` event (so the unreachable-but-configured case is loud, per REVIEW.md R14), and every gate event carries `studio.dispatch.{node,role,reason,xcode_version}` so debug sessions can distinguish local-red from mini-red — and a fallback-because-the-mini-was-down from a deliberate local pin — without guessing. The first remote gate invocation per session/node starts `scripts/node-warmup.sh` in the background, then continues normally. Remote source sync defaults to `NODE_SOURCE_SYNC_MODE=auto`: first dispatch for a session/path does a full rsync, later dispatches use git-diff selective rsync; set `NODE_SOURCE_SYNC_SMOKE=1` to dry-run compare against a full mirror and fall back on divergence. Successful remote Xcode build/test events also carry the node's `remote_derived_data_path`; set `NODE_ARTIFACT_RETRIEVE=1` to pull remote `.xcarchive` / `.xcresult` directories back to local DerivedData. See `_shared/contracts/events.md §Dispatch events` for the field shape.
