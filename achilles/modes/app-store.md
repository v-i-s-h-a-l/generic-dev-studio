...
writes:
  - plans/releases/<release-id>.yaml               # canonical (schema: _shared/schemas/release.md, release@1.0.0, channel: appstore)
...
1. Read prior releases. `scripts/query-plans.sh --kind=release --channel=appstore` enumerates prior App Store releases from `plans/releases/*.yaml`.
2. Find the last App Store entry (if any). Note its `tag`, `commit_sha`, and `released_at`.
3. Collect shipping tasks the same way as TF1 in the TestFlight release mode, but scoped to the App Store release range. Query `scripts/query-plans.sh --kind=task --state=verified,merged` and filter by merge-SHA ancestry against the previous App Store commit_sha.
4. Also record the previous release tag (for the release artifact's `tag` lineage). If no prior App Store release exists, use the oldest available tag matching `^[0-9]+-zaps$`.
