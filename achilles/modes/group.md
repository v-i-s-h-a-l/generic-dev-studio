...
1. **Resolve the group.** `scripts/query-plans.sh --kind=task --group=<group>` against `plans/index.yaml`. Find the task and all sub-tasks with the same `Group:` value. Sort: implementation first, then test-unit, test-integration, test-ui.
