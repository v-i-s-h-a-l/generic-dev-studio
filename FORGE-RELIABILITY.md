...
| Rank | Issue | Why Now |
|---:|---|---|
| 1 | [#224](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/224) Argus → merge gate policy + sibling-merge race | Finish the base-SHA recheck after the approved-only gate landed. |
| 2 | [#223](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/223) Achilles ↔ Argus contract hardening | Tightens timeout, staleness, and stage-handoff contracts. |
| 3 | [#322](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/322) Add provider model catalog and reviewer model policy | Wire the catalog/policy into PR reviewer selection after the source of truth landed. |
| 4 | [#372](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/372) Codex host must match Claude auth and test-context parity | Makes host-portability claims fail preflight instead of mid-task. |
| 5 | [#76](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/76) Audit mode-pack dual-write during Phase 2.6 transition | Close or retire the stale transition tracker after YAML-only migration evidence. |
| 6 | [#206](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/206) Communication-files architecture revamp | Removes remaining `legacy_artifact_read` compatibility surfaces. |
| 7 | [#225](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/225) Merge ↔ debrief atomicity | Prevents merge/debrief state from splitting across success boundaries. |
| 8 | [#243](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/243) Surface direct-merge-to-protected-branch daily count | Keeps ungated integration visible without manual event-log inspection. |
| 9 | [#340](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/340) Standardize task completion summaries across agent workflows | Makes completion evidence comparable across agents and hosts. |
| 10 | [#333](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/333) Harness/event-log loss reliability follow-up | Prevents local harness failures from erasing the canonical day log. |

## Active Chain: Reliability freeze closure

Label: `chain/reliability-closure`

Resolve in this order unless an urgent release fix is explicitly pulled forward:

Closed in this chain:

1. [#313](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/313) — deploy `hosts/registry.yaml` anywhere dispatch scripts resolve host manifests.
2. [#336](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/336) — require canonical task↔brief parity before dispatch.
3. [#335](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/335) — make debrief YAML the only active source for test cases.
4. [#240](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/240) — mint tasks for `follow_ups[]` and debt flags idempotently.
5. [#384](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/384) — normalize sweep enumerator kinds before ingest.

Remaining in this chain:

1. [#224](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/224) — finish sibling base-divergence recheck after review.
2. [#223](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/223) — finish Achilles↔Argus timeout, staleness, stage payload, and marker cleanup.
3. [#322](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/322) — connect reviewer policy selection to PR autopilot after catalog/policy landed.
4. [#372](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/372) — add Codex auth/test-context parity preflights.
5. [#76](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/76) — close as superseded only after the full mode-pack/runtime audit evidence is refreshed.

Chain goal: close the reliability freeze with dispatch, review, sweep, model selection, and host-portability failures surfacing as explicit blocks or unknowns, never silent success.
...
| [#313](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/313) Argus dispatch fails when host registry is missing from deployed skills layout | Closed | Reliability bug | Review gate availability depends on host registry deployment and deterministic infra-failure handling. |
| [#336](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/336) Brief resolution must be identical across deployed skills layouts | Closed | Reliability bug | Dispatch must resolve the same canonical brief on every supported host layout. |
| [#335](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/335) Make test cases come from debrief YAML only | Closed | Reliability bug | Verification must not depend on hidden legacy markdown authority. |
...
| [#322](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/322) Add provider model catalog and reviewer model policy | Open | Safety-floor hardening | Reviewer independence and model freshness should be policy, not stale shell defaults. |
| [#384](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/384) Sweep ingestion should normalize enumerator debrief kinds | Closed | Reliability bug | Enumerator/ingester vocabulary drift can strand artifacts while reporting success. |
| [#372](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/372) Codex host must match Claude auth and test-context parity | Open | Reliability bug | Codex-backed work should fail preflight when auth or test context differs from Claude. |
