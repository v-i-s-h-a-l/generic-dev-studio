...
| [#454](https://github.com/v-i-s-h-a-l/generic-dev-studio/issues/454) Phase-gate sibling review must use reviewer wrapper, not raw claude -p | Open | Reliability bug | Multi-phase work must use smoke-gated reviewer profiles instead of brittle raw host commands that bypass auth/config isolation. |
